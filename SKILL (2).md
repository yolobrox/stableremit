...
Upload a document (PDF, JPG, PNG, etc.) required for cross-border quotes. Returns a `file_id`.

```typescript
const fs = await import('fs');
const file = new File([fs.readFileSync('./invoice.pdf')], 'invoice.pdf', { type: 'application/pdf' });
const doc = await client.uploadDocument(file, 'Payment_Invoice');
// Returns: { file_id: '...', file_name: 'invoice.pdf' }
```

### 5. Request a Quote
Get an FX quote for a cross-border payment. Quotes expire — confirm quickly. Requires a `supporting_document_id`.
...
const quote = await client.requestQuote({
  ...
  supporting_document_id: doc.file_id
}, crypto.randomUUID());
...
2. **Upload a supporting document** (invoice, contract, etc.) — required for cross-border quotes
3. **Request a quote** with the beneficiary ID, amount, currency, and `supporting_document_id`
4. **Review the quote** — check `fx_rate`, `fees`, `total_debit_usd`, and `expires_at`
5. **Confirm the payment** using the `quote_reference`
6. **Monitor status** via `getPayment()` or `listPayments()`
